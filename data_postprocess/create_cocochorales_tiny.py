...
if __name__ == '__main__':
    ...
    for split in splits:
        for folder in folders:
            ...
            # get all the tars
            tars_all = glob.glob(os.path.join(cocochorales_full_dir, folder, split, '*.tar'))
            tars_to_move = tars_all[::(len(tars_all) // num_tars[split])]  # take every tar in equidistant intervals.
            for tar in tars_to_move:
                shutil.move(tar, os.path.join(output_dir, folder, split))
